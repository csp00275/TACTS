...
ai_buffer g_threeline_data_map_activations[AI_THREELINE_DATA_ACTIVATIONS_COUNT] = {
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 2752, 1, 1),
    2752, NULL, NULL),    /* heap_overlay_pool */
  };
ai_buffer g_threeline_data_map_weights[AI_THREELINE_DATA_WEIGHTS_COUNT] = {
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 227920, 1, 1),
    227920, NULL, s_threeline_weights_array_u64),   /* weights_array */
  };
...
ai_buffer ai_threeline_data_activations_buffer_get(const ai_handle ptr)
{
  ai_buffer buf = AI_BUFFER_INIT(
    ...
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, AI_THREELINE_DATA_ACTIVATIONS_SIZE, 1, AI_THREELINE_DATA_ACTIVATIONS_COUNT),
    AI_THREELINE_DATA_ACTIVATIONS_SIZE,
    NULL, ptr);
  return buf;
}
...
ai_buffer ai_threeline_data_weights_buffer_get(const ai_handle ptr)
{
  ai_buffer buf = AI_BUFFER_INIT(
    ...
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, AI_THREELINE_DATA_WEIGHTS_SIZE, 1, AI_THREELINE_DATA_WEIGHTS_COUNT),
    AI_THREELINE_DATA_WEIGHTS_SIZE,
    NULL, ptr);
  return buf;
}
...
{
  return AI_HANDLE_PTR(g_threeline_weights_table);
